...
export const UploadForm = ({ moduleId, onUploaded }: UploadFormProps) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      if (values.type === "link") {
        const res = await fetch(`/api/course-modules/${moduleId}/link`, {
          method: "POST",
          body: JSON.stringify({
        ...
        <div className="font-bold flex flex-wrap items-center justify-between gap-1 text-slate-800 mb-2">
          ...
          >
            {isEditing ? (
              ...
              <>
                <Upload size={16} className="mr-1" /> Subir material
              </>
            )}
          </Button>
        </div>
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        type="file"
                        accept=".pdf,.doc,.docx,.txt,.pptx,.xlsx,.xls,.csv,.zip,.rar,.png,.jpg,.jpeg"
                        onChange={(e) => {
                          if (e.target.files?.[0])
